...
const cardList = deck.cards.map(card => {
  return <div className="card w-100" key={card.id}>
  <div className="card-body">
    ...
    <Link to={`/decks/${deckId}/cards/${card.id}/edit`}>
              ...
              Edit
                ...
                <button className="btn  btn-secondary" type="button" onClick={() => cardDeleteHandler(card.id)}>
              Delete
                </button>
    </div></div></div>
        ...
        <div className="card w-75">
          <div className="card-body">
            ...
            <p className="card-text">{deck.description}</p>
              ...
              <button className="btn btn-danger" type="button" onClick={() => deckDeleteHandler(deck.id)}>Delete</button>
          </div>
        </div>
      </div>
      <div>
